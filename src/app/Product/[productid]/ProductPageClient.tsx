...
export default function ProductPageClient({ productid }: Props) {
  ...
  const currentpath = decodeURIComponent(productid);
  ...
  useEffect(() => {
    setLoading(true);
    const unsubscribe = onValue(ref(db as Database), (snapshot) => {
      try {
  ...
  return (
    <>
      {information ? (
        <section className={style.product_page}>
          <div className={style.wrapper}>
            {/* h1 для SEO — бачить Google */}
              ...
              <div className={style.product_visual}>
                  ...
                  {information.albom[0] != "" ? (
                    ...
                    >
                      {information?.albom.map((image: string, index: number) => (
                        <SwiperSlide
                          className={style.vertical_slide}
                          key={index + information.albomName[index]}
                        >
                          <Link href={"/Product/" + information.albomName[index]}>
                            <img src={image} alt={`Фото ${information.name} ${index + 1}`} />
                          </Link>
                        </SwiperSlide>
                      ))}
                ...
                <div className={style.main_image_slider}>
                  ...
                  <div className={style.capacity}>
                    {information && (
                      <>{`${information.capacity} ${t(information.unit)}`}</>
                    )}
                  </div>
                </div>
              </div>
